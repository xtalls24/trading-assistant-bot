...
class Database:
    ...
    # --- calendar notification tracking (dedupe H-2 / H-1 alerts) ---

    def mark_sent(self, event_id: str, which: str) -> None:
        with self.conn() as c:
            row = c.execute(
                "SELECT event_id FROM notifications WHERE event_id=?", (event_id,)
            ).fetchone()
            ...
            if not row:
                ...
                column = "h2_sent" if which == "h2" else "h1_sent"
                c.execute(
                    f"UPDATE notifications SET {column}=1, last_checked=? WHERE event_id=?",
                    (now, event_id),
                )

    def get_notification_status(self, event_id: str) -> Optional[Dict[str, Any]]:
        with self.conn() as c:
